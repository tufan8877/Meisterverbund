...
export function NewsDetailPage({ slug }: NewsDetailPageProps) {
  const { data: post, isLoading, isError } = useGetNewsPost(slug);
  ...
  if (isError || !post) {
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12 text-center">
        <h1 className="text-xl font-bold mb-3">Meldung nicht gefunden</h1>
        <Link href="/news" className="text-primary hover:underline">Zuruck zu News</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
      <Link href="/news" className="inline-flex items-center gap-1 text-muted-foreground hover:text-primary text-sm mb-6 transition-colors">
        <span>←</span> Alle News
      </Link>
      ...
      <article>
        ...
        <div className="mb-6 pb-6 border-b border-border">
          <StarRating contentType="news" contentId={post.id} />
        </div>

        <p className="text-lg text-muted-foreground leading-relaxed mb-6 italic border-l-4 border-accent pl-4">{post.excerpt}</p>
      ...
      <CommentSection contentType="news" contentId={post.id} />
    </div>
  );
}
